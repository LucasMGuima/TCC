...
Uma destas aplicações são os modelos de distribuição de espécies, que são capazes de fornecer uma visualização 
da situação da fauna e flora de determinada região, podendo mostrar como estas estão se comportando no 
decorrer do tempo \cite{speciesDistributionModels}.
...
\begin{figure}[H]
    \centering
    \caption{\label{fig:reg_linear_multipla}Regressão Linear Multipla}
...
$wn$ pontos, o $i$-ézimo ponto mais $\frac{(wn - 1)}{2}$ pontos em cada lado do $i$-ézimo ponto. Assumindo que os dados
...
$f(x_1,...,x_n)$ sobre um dominio $D$ de interesse. Onde MARS provê uma abordagem natural para a modelagem de variáveis
...
Assim conseguimos chegar a seguinte análise do algoritmo \ref{fig:insertion_sort}:
...
e execute $n$ vezes ira contribuir com $c_i \cdot n$ para o tempo total de execução. Para encontrar o tempo de execução do insertion-
...
dos dados iramos usar \verb|Overall ~ s(Income, bs = "cr")|.  
...
Neste cápitulo sera abordado a arquitetura e o preocesso de desenvolvimento de uma aplicação avaliadora de possiveis modelos
computacionais preditivos. Abrangendo as ferramentas e bibliotecas utilizdas, algoritmos desenvolvidos e a execução do projeto.

\section{Arquitetura}

A arquitetura proposta para a aplicação, foi idealizada para ser robusta e adaptavel, tornando a aplicação utilizavel com e sem
conexão a rede de internet, assim como a possivel inserção de novos modelos preditivos.
Sua estrutura segue como referencia o seguinte esquema, que pode ser encontrado no Anexo \ref{anexo:arquitetura_sistema}.
\begin{figure}[H]
    \centering
...
No sistema idealizado, a parte de controlo da aplicação é responsabilidade da classe App, a classe de \textit{call\_model},
age como uma interface entre o App e os modelos disponíveis, responsavel por informar quais modelos estão disponíveis, 
rodar seus respectivos treinos e teste.

Está mesma interface possibilita a adição de novos modelos, sendo necessário se adequar ao modo como os dados são passdos e
como o sistema espera encontrar seus modelos treinados e respectivas respostas.

Os modelos implementandos possuem um processo de treino semelhante, seus fluxos são respresentados nos sequintes
...
Tanto para os modelos disponíveis quanto para os dados, o sistema possue a capacidade de armazenar os mesmo em um serviço de armazenamento
pela internet quanto em um armazenamento local. Tornando o mesmo independente de uma conexão contínua com a internet.

\section{Recursos tecnologicos}
Nesta seção é apresentado os recursos utilizados para a confecção da aplicação, assim como uma breve justificativa em relação a escolha 
de cada uma.
\begin{itemize}
  ...
  Máquina e Inteligência Artificial. Esta linguagem foi escolhida pela suas bibliotecas que facilitam o tratamento de dados, e a
  ...
  de Máquina, mas se destaca também no campo da Análise Estatística. Esta linguagem foi escolhida pela sua implementação robusta dos
  modelos GAM, GLM e MARS.
  \item Streamlit 1.48.1: Biblioteca para a linguagem Python, desenvolvida com o intuito de facilitar a criação e compartilhamento
  ...
  PostgreSQL, APIs instantâneas, Autenticação e Armazenamento. Escolhida pela sua capacidade de Armazenamento e a fácil integração com a
  linguagem Python. 
  \item subprocess: Biblioteca nativa do Python, possibilita que durante a execução de um \textit{script} Python, seja chamado outro
...
O sistema se encarrega de fazer um leve tratamento dos dados a serem usados, não utilizando de entradas com dados faltantes que podem
atrapalhar na criação dos modelos e seus respectivos testes. Para a separação dos dados, o sistema os divide me dois gurpos, dados
para teste e dados para treinos, de um modo aleatorio, separando 80\% dos dados para treino e 20\% dos dados para teste.

Os dados são utilizados no sistema para inferir quais testes devem ser usados para avaliar o modelo, visto que, se os mesmo forem dados
continuos as métricas avalaitivas são diferentes das usadas na situação onde os dados são categoricos. Assim como o nome das colunas é 
usado para selecionar quais serão os parametros dependente e independentes.
...
Para os modelos, o seu armazenamento ocorre de modo semelhante, quando em um senario local, são ecnontrados na pasta \textit{models}, e
e quando em um funcionamento online, os mdoelos são encontrados no \textit{storage}, porém na pasta \textit{models}.

A aplicação sempre tenta entrar em modo online, tentando se conectar com o \textit{Supabase}, se falhar, inicia altomaticamente em
modo offline, e passa a procrar os arquivos em suas pastas locais, do contrário, sempre busca no \textit{storage} configurado no 
\textit{Supabase}.

\section{Implementação}

O aplicativo foi majoritariamente desenvovlido utilizando da linguagem de programação Python,
e os modelos são implementados utilizando a linguagem de programação R, assim como os metodos
de avaliação.

O parte principal da aplicação foi implementada no arquivo \textit{app.py}, onde é tratado a
conexão com o \textit{Supabase} assim como o preparo para o treino de um novo modelo.

A conexão é tratada no seguinte pedaço de código.
...
Onde se não for possivel a conexão com o Supabase por qualquer motivo,
o aplicativo é iniciado no modo offline, utilizando dos dodos e modelos
disponíveis localmente.

A parte visual do aplicação é gerenciada pela biblioteca \textit{Streamlit}.

A utilização dessa biblioteca pode ser visto nas função utilizadas para gerar
os campos de seleção que serão variados dependendo do conteudo disponivel.
...
Onde o funcionamento da biblioteca \textit{Streamlit} esta encapsulado pelos script
\textit{widgets}, para ver com maior detalhes, podemos usar a função utilizada no
...
reposta e os preditores. Isso está implementado na seguinte parte do programa.
...
Tendo os parâmetro selecionados, o treino do modelo pode ser efetuado. Para isso
é necessário utilizar das funções implementadas dentro do script \textit{call\_model}.
...
Tendo os modelos treinados, a lógica dos teste começa no script \textit{pages\\testes.py}. Este
que por sua vez, chama o script em R responsavel por gerenciar os testes, atravez da
...
Por sua vez, esse script em R, identifica o qual metodo de avalaiação será usado,
com base no tipo de dados, que é inferido apartir da porcentagem de dados valores
unicos e o tipo de classe da coluna sendo avalaiada.

Tento avalido os modelos, o script retorno ao aplicativo as metricas usadas e seus
respectivos valores.

\subsection{Modelos}

Para se gerar um modelo, primeiro precisamos escolhar qual dataset será utilizado. E qual o tipo de modelo a ser gerado, aqui é possivel
escolher mais de um tipo de modelo, para crianção em conjunto. 

O código para gerar um modelo, possui alguns pontos importantes. Como ele recebe os dados para se treinar o modelo e como deve ser
o retorno para o aplicativo. As informações necessárias são passadas atravez de argumentos de comando, e para extrai-los, usamos a
seguinte parte do scritp em R.
...
Tendo os dados e o tipo do modelo sido escolhido o sistema nos informa quais os parametros podem ser utilizados como preditores e 
repostas, permitindo a escolha de um número variado de váriaveis em ambos os casos.

Sendo de responsabilidade do scirpt do que gera o modelo, criar a equação para a geração do modelo, tomando como exemplo temos o
...
Neste pedaço de codigo, montamos a equação que é utilizada pelo modelo para gerar a regressão linear utilizada para fazer
as inferencias necessárias. Assim como tratamaos os nomes e sertificamos que a equação esteja de acordo com oque é
esperado pela função que treinarar o modelo.

Durante o treino do modelo, os dados são separdos em dois conjunto, um pra o treino em si e o segundo para os testes posteriores, se estes
dados já não tiverem sido separados durante a criação de outros modelo. Ao terminar de criar, a aplicação gera um arquivo \textit{JSON} 
contendo os dados do modelo, como apresentado a baixo.
...
Onde, \textit{model} identifica onde esta salvo o modelo gerado, \textit{response\_name} a(s) variavel(is) de repostas, 
\textit{predictors\_name} os preditores usados separados por "\textbar" e \textit{data\_name} onde se encontra os dados a serem usados 
para teste.
...
Para avaliar o modelo, primeiro a aplicação identifica qual o tipo de dados que seram
utilizados, se são categoricos ou continuos. A patir dessa indentificação o aplicativo
escolhe quais as metricas de avaliação melhor se adequão para o teste.

A parte do script responsavel pela indentificação é encontrada codificada em R no seguinte
código.
...
Com os dados armazenados nas váriaveis \textit{percent\_unic} e \textit{col\_class} e scritp em
R infere com qual tipode dados estamos trabalhando, e descicde qual procedimento de avaliação
a ser efetuado.
...
Sendo para dados continuos, utilizadas as metricas de MAE e RMSE, e para dados categoricos
...
Deste modo, se é possivel identificar qual modelo escolhido atua melhor com os dados
...
Para identifcar a viabilidade da aplicação, foi efetuado teste utilizando dois conjuntos de dados 
distintos, sendo um deles utilizado para verificar a capacidade avaliativa quando tratamos de dados
continuos e o outro para avaliar quando temos dados categoricos.

Para os dados continuos, foi utilizado o conjunto de dados, \textit{psiasci2006.csv}, este conjunto 
...
O modelo gerado apartir destes dados, utilizou como parametros os valores de, \textit{Issue}, \textit{Interest},
\textit{Support}, \textit{Income}, \textit{Health} e \textit{Edu}, para estimar o valro de \textit{HDI}.
Os resultados encontrados para esse caso foram. 

\begin{table}[H]
    \centering
    \caption{\label{tab:resultados_continuos} Metricas de um modelo continuo}
    \begin{tabular}{|c|c|c|}
        \hline
...
tem como localidade a cidade de São Paulo encontradas em um dataset maior, o \textit{data\_2025.txt}, um dataset
fornecido pelo site \textit{eBird}, contendo avistamentos de aves em um parametro mundial.

Neste caso os modelos gerados usaram como parametros so valores de, \textit{Latitude}, \textit{longitude} e
...
Assim como identificar o principal problema com dados categoricos que atualmente não
retornam as metricas geradas em seus respectivos testes.
