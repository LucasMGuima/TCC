...
o erro qie se comete ao estimar os valores de $Y$ por meio da variável $X$ \cite{modelos_regressao_linear}.
...
podemos trabalhar com conjuntos de valores, que agrupam valores de $Y$ distintos, para cada valor de $X$.
...
Onde:
\begin{itemize}
	\item ($i$) representa um agrupamento de dados, um estado brasileiro seguindo o exemplo dado a sima;
...
váriavel de resposta, este que podem também ser correlacionados com a váriavel independente. A formula
parar este modelo de regressão, pode ser representa da da seguinte forma, onde temos $k$ variáveis, 
...
A acurácia é um modo de avaliar s a performance de um modelo, assim identificando se seus resultados podem
...
ser usados para entender e/ou prever a distribuição de uma espécie por uma dada locaclidade \cite{speciesDistributionModels}.

SDMs comtemporaneos combinan conceitos de ecologia e historia natural com os avanços mais recentes em estatísticas e
tecnologia da informação, as raizes destes modelos são encontradas no estudos mais antigos que descrevem padrões
biológicos em termos de relações com geografia e/ou gradientes ambientais, e estudos que indicam a resposta individula
de especies para seus ambientais prove um forte argumento conceitual para se modelar especies de modo individual 
...
\begin{itemize}
	...
	\item Dado ambiental: São a descricção do ambiente, podendo conter medições de temperatura e precepitação, como
...
Generalized Linear Models agrupão uma grande quantidade de modelos discretos e continuos, particularmente util para se 
...
Ao contrário dos classicos modelos lineares, que propoem uma distribuição Gaussiana (normal) e uma função de ligação dos
valores de $X$ e $Y$, os GLMs permitem que a função de distribuição seja qualquer uma da família de distribuições 
exponenciais, Gaussiana, Poisson ou Binomial, e a função de ligação pode ser qualquer função função monotonica 
...
Para se fazer o fiting do modelo, MARS se utiliza de splines, sendo a o metodo mais popular o "B-Spline" usado em
conjunto com o fiting por ultimo-quadrado. O processo de spline, consiste em separa os dados nos
...
de dados, ou de espaço, quando da memoria do computador o algoritmo ira usar durante seu processo.
...
apresentadas a cima, reduzindo a forumla a sua caractereistica mais presente, visto que aqui assumimos que $n$ sejá
um valor muito grande. Por exemplos uma função que tenha a forma de $n^2 + n + c$, onde $c$ é uma constante, 
pode ser reduzida a $n^2$, já que esté tera maior peso durante a computação, fazendo está ser caracterizada 
como $O(n^2)$, onde $O$ é a notação "grande O", que representa a complexidade do algoritmo \cite{introductionAlgorthms}.
...
cega, podendo perder em tempo e consumo desnecessário de memoria.

\subsection{Análise de Complexidade}

Na análise de complexidade, estimamos o tempo de execução de um algoritmo dado um entrada de tamanho $n$, análisando 
seus comandos, como exemplo podemos utilizar o algoritmo de ordenação bubble-sort. O bubble-sort é algorimo que ordena
uma sequência de números, encontrando onde o valor atual se encaixa na sequencia, sequindo os sequintes passos:
...
Apartir do psceudo código a cima, conseguimos realizar a análise de complexidade. Na linha 2 "$for i = 0 to n-1 do$"
...
No final, facaremos com uma complexidade de $O(n^2)$ visto que percorremos o vetor de entrada duas vezes, e este ponto
será oque apresentara maior impacto no desempenho com vetores de tamanho $n$ muito grande.
...
matemática, assim como ná análise anterior, como exemplo tomemos o algortimo bubble-sort.

No algoritmo ded bubble-sort, temos as seguintes váriaveis:
...
Assim, o algoritmo não cria nenhuma variavel a mais durante sua execução, acarretando em um custo de mormia costante durante
