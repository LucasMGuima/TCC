...
\begin{siglas}
  ...
  \item[SDM] Modelo de Distribuição de Especie 
\end{siglas}
...
% [x] Metodos de avaliação de modelo
...
O estudo "A comprehensive evaluation of predictive performance of 33 speceis distribution modelis at species and community levels"
\cite{predPerform33models} teve como foco a avaliação de 33 modelos distintos de distribuição de especies, se questionando
performance de cada um trabalhando com comunidades e com especies especificas.

Neste trabalho, os autores, identificam os parametros de cada modelo, e seus tipos, mostrando quais são esperados que trabalhem melhor
com comunidades ou com especies únicas, indeificando dentro dos modelos selecionadas que os cinco melhores são os HMSC.3, GLM.5,
MISTN.1, MARS.1 e GNN.1.

Onde performance final é influenciada por três fatores, o modelo escolhido, o foco de predição e a qualidade dos dados, e o modelo
que obiteve a melhor performance quando considerado todas as especeis, comunidade, foi o HMSC.1 e para apenas uma qauando apenas um 
especies foi levada em consideração o modelo GLM.4 teve uma performance melhor.

Podemos ver neste trabalho dois dos três tipos de modelos de SDM mais comunso, GLM (GLM.4 e GLM.5) e MARS (MARS.1), mostrando que
além do acesso relativamente facil a estes e de possuirem um software bem documenta e amigavel \cite{predPerform33models}, sua 
qualidade de performance ajuda em serem modelos relevantes.

No estudo "Generalized linear and generalized additive models in studies of species distributions: setting the scene" 
\cite{GAMeGLM_especie_estudo}, os autores neste fazem uma breve revisçao sobre modelos lineares, GLMs e GAMs. Apresentando algumas
de esuas caractereisticas e as relações entre os mesmos.

Uma descirção mais teorica dos modelos GLM, GAM e MARS são encontradas nos sequintes trabalhos, respecitivamente, "The generalized 
...
Nestes temos uma dicertação sobre a parte matematica dos modelos, como é feito a inferencia de cada um de seus parametros e como o mesmo
atua sobre os dados de entrada. Em alguns temos exemplos de uso de cada modelo, e suas variações mostrando onde estas são diferentes
do modelo que é tratado no estudo.
