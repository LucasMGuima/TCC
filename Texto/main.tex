...
Se assumirmos $x$ sendo um valor inteiro, que $wn$ é ímpart, então a span $w$ vizinhança mais próxima simétrica conterá
...
A vizinhança fica truncada proxima aos pontos finais se os pontos $\frac{wn-1}{2}$ não estão disponíveis. O span de $w$
...
uais considerar naquele passo, se repetindo até uma quantidade relativamente grande de funções bases for selecionada. 

Um conjunto final de funções base de tamanho apropriado é então selecionado por meio de um procedimento de seleção de subconjunto de variáveis passo a passo 
...
\begin{itemize}
	...
	que mesmo cada passo leva um tempo diferente de outras, o tempo da do $i$ passo é sempre uma constante, $c_i$.  
\end{itemize}
...
\begin{itemize}
	\item $Vtor A$ a sequência de $n$ inteiros;
...
que obteve a melhor performance quando considerado todas as espéceis (comunidade) foi o HMSC.1 e para apenas uma quando apenas um 
...
\cite{GAMeGLM_especie_estudo}, os autores neste fazem uma breve revisão sobre modelos lineares, GLMs e GAMs. Apresentando algumas
