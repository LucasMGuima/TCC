...
Está mesma intergace possibilita a adição de novos modelos, sendo necessário se adequar ao modo como os dados são passdos e
...
atrapalhar na criação dos modelos e seus respectivos testes. Para a separação dos dados, o sistema divide os dados em dois grupos, dados
para teste e dados para treinos, de modod aleatorio, separando 80\% dos dados para treino e 20\% dos dados para teste.
...
continuos as métricas avalaitivas são diferentes na situação onde os dados são categoricos. Assim como o nome das colunas é usado para
selecionar quais serão os parametros dependente e independentes.
...
encontrados no \textit{storage} data dentro da pasta \textit{dados}. 
...
e quando estamos em um funcionamento online, os mdoelos são encontrados no \textit{storage} data, porém na pasta \textit{models}.

A aplicação sempre tenta entrar em modo online, tentando se conectar com o \textit{Supabase}, se falhar ela se inicia altomaticamente em
...
Ao final do processo de avaliação o modelo retorno todas os resultados obtidos, sendo
possível efetuar o teste em vários modelos gerados simultaneamente.
